...
const OtherInfo = React.memo(({ otherInfo }: { otherInfo: any }) => {
  const apiConfig = useApiConfig();
  const {
    motherTongue,
    ...rest
  } = otherInfo;
  if (otherInfo.hasOwnProperty("otherInfoId")) delete otherInfo.otherInfoId;
  const formatField = (key: string, value: string | any) => {
    switch (key) {
      case "motherTongueId":
        return { label: "Mother Tongue", value: motherTongue?.motherTongueName };
      default:
